...
object UiFactory {
		...
		if (sharedPixelFont == null) {
			...
			val handle =
				com.badlogic.gdx.Gdx.files
					.classpath("fonts/boldpixels/BoldPixels.ttf")
			val generator =
				com.badlogic.gdx.graphics.g2d.freetype
					.FreeTypeFontGenerator(handle)
			val params =
				com.badlogic.gdx.graphics.g2d.freetype.FreeTypeFontGenerator.FreeTypeFontParameter().apply {
					size = 24
					color = Color.WHITE
			...
			sharedPixelFont =
				generator.generateFont(params).apply {
	...
	fun createLabelStyle(
		font: BitmapFont,
		color: Color = Color.WHITE,
	): Label.LabelStyle = Label.LabelStyle(font, color)
